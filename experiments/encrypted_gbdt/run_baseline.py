"""
Run plaintext XGBoost baseline experiments.

This script trains standard GBDT without privacy features
for comparison with Guard-GBDT.
"""
...
from utils.data_loader import load_phishing_data, partition_data_vertical, create_realistic_phishing_data
from utils.metrics import evaluate_model, print_metrics
from models.plaintext_gbdt import PlaintextGBDT


def run_baseline_experiment(n_estimators: int = 100,
                           max_depth: int = 6,
                           learning_rate: float = 0.1,
                           n_parties: int = 3,
                           random_state: int = 42) -> dict:
    """
    Run baseline plaintext GBDT experiment.

    Args:
        n_estimators: Number of trees
        max_depth: Maximum tree depth
        learning_rate: Learning rate
        n_parties: Number of parties (for data partitioning)
    ...
    """
    print("=" * 60)
    print("Plaintext GBDT Baseline Experiment")
    print("=" * 60)
    ...
    model.fit_dict(X_train_dict, y_train, verbose=True)
    training_time = time.time() - start_time

    # Evaluate
    print("\nEvaluating on test set...")
    metrics = evaluate_model(model, X_test_dict, y_test)
    print_metrics(metrics, "Plaintext GBDT")

    # Get training stats
    stats = model.get_training_stats()

    results = {
        'model_name': 'PlaintextGBDT',
        'n_estimators': n_estimators,
        'max_depth': max_depth,
        'learning_rate': learning_rate,
        'n_parties': n_parties,
